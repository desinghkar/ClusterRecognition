...
//  memcpy (&p.ptr ()[0], &model.second[0], p.cols * p.rows * sizeof (float));
  memcpy (p[0], &model.second[0], p.cols * p.rows * sizeof (float));
...
main (int argc, char** argv)
{
  ...
  for (int i = 0; i < 1; ++i)
  {
	if(k_distances[0][i]<150)
	{
	string str = models.at(k_indices[0][i]).first.c_str();
	size_t found;
	found=str.find("ID_1");
	if(found!=string::npos)
		cout<<"Pulpy orange bottle ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_2");
	if(found!=string::npos)
		cout<<"Tyre ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_3");
	if(found!=string::npos)
		cout<<"Multimeter ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_4");
	if(found!=string::npos)
		cout<<"Water mug ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_5");
	if(found!=string::npos)
		cout<<"Water bottle ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_6");
	if(found!=string::npos)
		cout<<"Maaza tetra pack ["<<k_distances[0][i]<<"]"<<endl;
	found=str.find("ID_7");
	if(found!=string::npos)
		cout<<"Coffee Mug ["<<k_distances[0][i]<<"]"<<endl;
	}
	else
	{
